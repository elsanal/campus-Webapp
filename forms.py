...
class JobForm(FlaskForm):
    ...
    internship = TextAreaField('internship or job?', validators = [DataRequired(),])
...
class ScholarshipForm(FlaskForm):
    ...
    bachelor = StringField('bachelor?', validators = [DataRequired(),])
    master = StringField('master?', validators = [DataRequired(),])
    PhD = StringField('PhD?', validators = [DataRequired(),])
    logo = FileField('upload logo', validators = [FileAllowed(['jpg', 'png', 'jpeg',])])
    web = StringField('Website url', validators = [DataRequired(), Length(min=2)])
    how_to_apply_text = TextAreaField('How to apply text', validators = [DataRequired(),])
    how_to_apply_video = StringField('How to apply video url', validators = [DataRequired(),])
    popular = StringField('is it popular?', validators = [DataRequired(),])
